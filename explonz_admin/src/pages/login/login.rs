...
pub fn LoginPage() -> impl IntoView {
    ...
    let redirect_path = move || query.read().get("redirect").unwrap_or_default();
    let reason = move || query.read().get("reason").unwrap_or_default();
    ...
    view! {
            {
                move || {
                let status = AuthStatus::from_str(&reason());

                let msg = match status {
                    Ok(AuthStatus::TokenExpired) => Some("登录已过期，请重新登录"),
                    Ok(AuthStatus::NotLoggedIn)  => Some("请先登录以继续"),
                    _                 => None,
                };
                msg.map(|text| view! {
